...
class DefineDf:
    def __init__(self, path: str, first_str_is_header=True):
        ...
        self._first_str_is_header = first_str_is_header
    ...
    def create_df(self):
        ...
        if self._type in ["csv", "tsv", "txt"]:
            ...
            if self._first_str_is_header:
                # Если есть заголовок
                self._df = pd.read_csv(self._path, sep=sep, header=0)
            else:
                # Если заголовка нет, читаем первую строку для определения числа столбцов
                df_temp = pd.read_csv(self._path, sep=sep, header=None, nrows=1)
                num_columns = df_temp.shape[1]  # Количество столбцов
                names = [f"col_{i}" for i in range(num_columns)]  # Генерируем имена
                self._df = pd.read_csv(self._path, sep=sep, header=None, names=names)

        else:  # Для Excel
            if self._first_str_is_header:
                self._df = pd.read_excel(self._path, header=0)
            else:
                df_temp = pd.read_excel(self._path, header=None, nrows=1)
                num_columns = df_temp.shape[1]
                names = [f"col_{i}" for i in range(num_columns)]
                self._df = pd.read_excel(self._path, header=None, names=names)


class LoadData(DefineDf):
    def __init__(self, path, first_str_is_header=True):
        super().__init__(path, first_str_is_header)
        self._types_with_sep = ["csv", "tsv", "txt"]
        try:
            self.control_data()
            self.create_df()
    ...
    def control_data(self):
        ...
        if self.type in self._types_with_sep and not self.check_data_in_file():
...
if __name__ == "__main__":
    ...
    ld = LoadData(
        f"{DIR}tests/datasets/data_colon.txt",
        first_str_is_header=False,
    )
